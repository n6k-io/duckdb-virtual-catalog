...
PhysicalOperator &ProviderTableCatalog::PlanInsert(ClientContext &context, PhysicalPlanGenerator &planner,
	...
	// Empty UDF name means the provider did not implement this op (see register_provider in provider.py).
...
PhysicalOperator &ProviderTableCatalog::PlanDelete(ClientContext &context, PhysicalPlanGenerator &planner,
	...
	auto pk_buffer = FindScanAndAttachPKBuffer(plan, table.table_info->primary_keys);
	if (!pk_buffer) {
		throw InternalException("vcat_provider: could not locate scan for PK buffer injection");
	}
...
PhysicalOperator &ProviderTableCatalog::PlanUpdate(ClientContext &context, PhysicalPlanGenerator &planner,
	...
	auto &upd_op =
	    planner.Make<ProviderTableUpdate>(table, op.columns, std::move(op.expressions),

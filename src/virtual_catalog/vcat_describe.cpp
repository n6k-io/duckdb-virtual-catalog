...
static unique_ptr<FunctionData> TableDescribeBind(ClientContext &context, TableFunctionBindInput &input,
	...
	if (!args.has_table) {
		throw BinderException("vcat_table_describe requires a \"table\" argument: "
		                      "vcat_table_describe('catalog', schema := 'main', \"table\" := 'foo')");
	}
	...
	if (!entry) {
		throw CatalogException("vcat_table_describe: table \"%s.%s.%s\" not found", args.catalog, schema_name,
		                       args.table);
	}
	...
	// PK/writeable/editable reuse the vcat_table_permissions collectors, filtered to this table.
...
void RegisterTableDescribeFunction(ExtensionLoader &loader) {
	TableFunction func("vcat_table_describe", {LogicalType::VARCHAR}, TableDescribeScan, TableDescribeBind,

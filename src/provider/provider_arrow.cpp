...
// Older providers emit this key instead. Read for compatibility; nothing writes it.
const char *const LEGACY_PRIMARY_KEY_METADATA_KEY = "n6k.primary_keys";

namespace {

// Reports the decoder's own error text, which names the nanoarrow failure.
...
void MakeStreamFromIpc(const string &ipc_bytes, ArrowArrayStream *out, const char *what) {
	...
	// A table with no rows decodes to no batches at all: a legitimate empty result, not a failure.
	ArrowArray batch;
...
void RowChunkBuilder::Append(const Value &value) {
	if (!current_) {
		throw InternalException("virtual_catalog_provider: RowChunkBuilder::Append before BeginRow");
	}
	if (column_ >= types_.size()) {
		throw InternalException("virtual_catalog_provider: more values appended than the row has columns");
	}
	// Cast rather than trust: a key value read out of an Arrow buffer carries that buffer's type,
	// which need not be the column's own.
...
int64_t CallWriteUdf(Connection &conn, const string &udf_name, const string &table_name, const string &arrow_ipc,
	...
	auto statement = conn.Prepare(vcat::UdfCallWithParams(udf_name, 2 + extra.size()));

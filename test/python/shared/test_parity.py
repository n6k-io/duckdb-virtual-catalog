...
@pytest.mark.parametrize(
    "backend_name",
    # The backend name is also the path marker, so `-m "parity and bridge"` selects one half.
    [pytest.param(name, marks=getattr(pytest.mark, name)) for name in sorted(BACKENDS)],
)
